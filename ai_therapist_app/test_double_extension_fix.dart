...
void testTTSWorkflow() {
  ...
  assert(
      !simulatedPath.endsWith('.wav.wav'), 'Should not have double extension');
  assert(simulatedPath.endsWith('.wav'), 'Should have single .wav extension');

  // Verify correct pattern (matches logs)
  assert(simulatedPath.startsWith('tts_stream_tts_'),
      'Should have correct prefix pattern');

  // Count .wav occurrences (should be exactly 1)
  final wavCount = '.wav'.allMatches(simulatedPath).length;
  assert(
      wavCount == 1, 'Should have exactly one .wav extension, found $wavCount');
...
void testPathManagerIntegration() {
  ...
  assert(
      finalPath == 'tts_stream_tts_12345.wav', 'Should generate expected path');
  assert(
      !finalPath.contains('.wav.wav'), 'Should not contain double extension');
