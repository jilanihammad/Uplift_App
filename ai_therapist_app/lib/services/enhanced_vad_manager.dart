...
class EnhancedVADManager {
    ...
    try {
      ...
      try {
        ...
        if (_rnnoiseInitialized) {
          if (kDebugMode) {
            AppLogger.d(
                ' Enhanced VAD ($_vadInstanceId): RNNoise initialized successfully');
          }
        } else {
          if (kDebugMode) {
            print(
      ...
      if (kDebugMode) {
        AppLogger.d(
            ' Enhanced VAD ($_vadInstanceId) initialized (RNNoise: ${_useRNNoise ? 'ENABLED' : 'DISABLED'})');
      }
    } catch (e) {
      _clearOperationTimeout();
  ...
  Future<bool> startListening() async {
    ...
    if (_isListening) {
      if (kDebugMode) {
        AppLogger.d(
            ' Enhanced VAD ($_vadInstanceId): Already listening, ignoring duplicate start');
      }
      return true;
    }
    ...
    if (kDebugMode) {
      AppLogger.d(
          ' Enhanced VAD ($_vadInstanceId): Starting voice activity detection');
    }
  ...
  Future<bool> _startRNNoiseVAD() async {
    try {
      ...
      try {
        ...
        if (kDebugMode) {
          AppLogger.d(
              ' Enhanced VAD ($_vadInstanceId): Audio streamer configured for ${_sampleRate}Hz (RNNoise requirement)');
        }
      } catch (e) {
        if (kDebugMode) {
          print(
      ...
      if (kDebugMode) {
        AppLogger.d(
            ' Enhanced VAD ($_vadInstanceId): Started RNNoise-based voice detection');
      }
  ...
  Future<bool> _startAmplitudeVAD() async {
    try {
      ...
      if (kDebugMode) {
        AppLogger.d(
            ' Enhanced VAD ($_vadInstanceId): Started amplitude-based voice detection');
      }
    ...
    try {
      ...
      if (processedFrame != null) {
        ...
        if (kDebugMode && (vadProbability > 0.1) && _shouldLog()) {
          AppLogger.d(
              ' Enhanced VAD (RNNoise): confidence=${vadProbability.toStringAsFixed(3)} | '
  ...
  bool _shouldLog() {
    ...
    if (_lastLogTime == null ||
        now.difference(_lastLogTime!) >= _logThrottleInterval) {
      _lastLogTime = now;
      return true;
    }
    return false;
