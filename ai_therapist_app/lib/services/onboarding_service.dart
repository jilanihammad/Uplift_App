...
class OnboardingService implements IOnboardingService {
  ...
  Future<void> init() async {
    ...
    debugPrint('OnboardingService init - hasCompleted: $_hasCompleted');

    if (_hasCompleted) {
      debugPrint('Onboarding already completed, setting step to complete');
      _currentStep = OnboardingStep.complete;
    } else {
      // Load current step if not completed
      final stepIndex = prefs.getInt(_currentStepKey) ?? 0;
      debugPrint('Loaded step index from SharedPreferences: $stepIndex');

      _currentStep = OnboardingStep.values[stepIndex];
      debugPrint('Set current step to: ${_currentStep.toString()}');
  ...
  Future<void> goToNextStep() async {
    ...
    debugPrint('To next step with index: $nextStepIndex');
    ...
    if (nextStepIndex < OnboardingStep.values.length) {
      ...
      debugPrint('To next step: ${nextStep.toString()} (index: ${nextStep.index})');
  ...
  Future<void> goToStep(OnboardingStep step) async {
    debugPrint('Requested to go to step: ${step.toString()} (index: ${step.index})');
  ...
  Future<void> _saveStep(OnboardingStep step) async {
    ...
    debugPrint('Current step in memory updated to: ${_currentStep.toString()}');
