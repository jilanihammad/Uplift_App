...
class OpusHeaderUtils {
  ...
  static const List<int> opusHeadSignature = [0x4F, 0x70, 0x75, 0x73, 0x48, 0x65, 0x61, 0x64]; // "OpusHead"
  
  /// OpusTags signature (8 bytes: "OpusTags")
  static const List<int> opusTagsSignature = [0x4F, 0x70, 0x75, 0x73, 0x54, 0x61, 0x67, 0x73]; // "OpusTags"
  ...
  static OpusHeaderInfo? parseOpusHeaders(List<int> data) {
    if (data.length < minHeaderBufferSize) {
      if (kDebugMode) {
        print('⏳ OpusHeaderUtils: Buffer too small for complete headers (${data.length} < $minHeaderBufferSize)');
      }
    ...
    for (int i = 0; i <= scanLimit - 8; i++) {
      ...
      if (opusHeadOffset == null && _matchesSignature(data, i, opusHeadSignature)) {
        opusHeadOffset = i;
        if (kDebugMode) {
      ...
      if (opusTagsOffset == null && _matchesSignature(data, i, opusTagsSignature)) {
        opusTagsOffset = i;
        if (kDebugMode) {
      ...
      if (opusHeadOffset != null && opusTagsOffset != null && opusDataOffset == null) {
    ...
    if (opusHeadOffset == null || opusTagsOffset == null) {
      if (kDebugMode) {
        print('⏳ OpusHeaderUtils: Missing required headers - OpusHead: ${opusHeadOffset != null}, OpusTags: ${opusTagsOffset != null}');
      }
      return null; // Need more data
    }
    
    // Calculate total header size
    final headerEndOffset = opusDataOffset ?? (opusTagsOffset! + 256); // Conservative estimate if no audio data found yet
  ...
  static Uint8List? extractOpusHeaders(List<int> data, OpusHeaderInfo headerInfo) {
    ...
    final headers = Uint8List.fromList(data.take(headerInfo.totalHeaderSize).toList());
    
    if (kDebugMode) {
      print('📦 OpusHeaderUtils: Extracted ${headers.length} bytes of OPUS headers');
    }
  ...
  static Uint8List extractOpusAudioData(List<int> data, OpusHeaderInfo headerInfo) {
    final audioStartOffset = headerInfo.audioDataOffset ?? headerInfo.totalHeaderSize;
    ...
    if (kDebugMode) {
      print('🎵 OpusHeaderUtils: Extracted ${audioData.length} bytes of OPUS audio data');
    }
  ...
  static bool _matchesSignature(List<int> data, int offset, List<int> signature) {
  ...
  static Uint8List combineOpusHeadersAndAudio(Uint8List headers, Uint8List audioData) {
    ...
    if (kDebugMode) {
      print('🔧 OpusHeaderUtils: Combined headers (${headers.length}B) + audio (${audioData.length}B) = ${combined.length}B total');
    }
