  ...
  void dispose() {
    ...
    try {
      ...
      if (this is AsyncDisposable) {
        ...
        if (kDebugMode) {
          debugPrint('[SessionDisposable] $runtimeType requires async disposal - use disposeAsync()');
        }
      } else {
    ...
    try {
      ...
      await Future.any([
        ...
        Future.delayed(const Duration(seconds: 5), () {
          throw TimeoutException('Disposal timeout for $runtimeType', const Duration(seconds: 5));
        }),
      ]);
      
      if (kDebugMode) {
        debugPrint('[SessionDisposable] $runtimeType async disposal completed in ${stopwatch.elapsedMilliseconds}ms');
      }
      ...
      if (kDebugMode) {
        debugPrint('[SessionDisposable] Error async disposing $runtimeType: $e');
      }
