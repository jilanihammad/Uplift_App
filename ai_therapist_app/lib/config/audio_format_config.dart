...
class AudioFormatConfig {
  ...
  static int get opusBitrate => _getEnvInt('TTS_OPUS_BITRATE', 64000); // 64 kbps default
  
  /// OPUS sample rate in Hz
  static int get opusSampleRate => _getEnvInt('TTS_OPUS_SAMPLE_RATE', 48000); // 48 kHz default
  
  /// OPUS channels (1 = mono, 2 = stereo)
  static int get opusChannels => _getEnvInt('TTS_OPUS_CHANNELS', 1); // Mono default
  
  /// OPUS rollout percentage (0-100)
  static int get opusRolloutPercentage => _getEnvInt('TTS_OPUS_ROLLOUT_PCT', 0); // 0% default
  ...
  static bool validateConfiguration() {
    ...
    if (enableOpusFormat && !enableOpusHeaderBuffering) {
      warnings.add('OPUS format enabled but header buffering disabled - may cause playback issues');
      isValid = false;
    }
    ...
    if (streamingBufferThreshold < 1024) {
      warnings.add('Streaming buffer threshold is very low - may cause excessive buffering');
    }
    
    if (opusHeaderTimeoutMs < 1000) {
      warnings.add('OPUS header timeout is very low - may cause premature timeouts');
    }
