...
typedef TimeUpdateCallback = void Function(int elapsedSeconds, int remainingSeconds);
...
class TimerManager {
  ...
  bool get isExpired => _sessionDuration != null && elapsedSeconds >= _sessionDuration!.inSeconds;
  
  /// Set the session duration
  void setSessionDuration(Duration duration) {
    if (kDebugMode) {
      debugPrint('[TimerManager] Session duration set to ${duration.inMinutes} minutes');
    }
  ...
  void startTimer() {
    ...
    if (_sessionDuration == null) {
      if (kDebugMode) {
        debugPrint('[TimerManager] Cannot start timer without session duration');
      }
      return;
    }
  ...
  void resumeTimer() {
    ...
    if (kDebugMode) {
      debugPrint('[TimerManager] Resuming timer from ${elapsedSeconds} seconds');
    }
  ...
  void _updateElapsedTime({bool forceEmit = false}) {
    if (_sessionStartTime != null) {
      ...
      if (forceEmit || totalElapsed.inSeconds > _accumulatedTime.inSeconds) {
        ...
        if (!_warningTriggered && remainingSeconds <= 300 && remainingSeconds > 0) {
          _warningTriggered = true;
          onTimeWarning?.call();
          if (kDebugMode) {
            debugPrint('[TimerManager] Time warning triggered - 5 minutes remaining');
          }
        }
