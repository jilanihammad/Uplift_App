...
class MessageCoordinator {
  ...
  TherapyMessage addUserMessage(String content) {
    if (kDebugMode) {
      debugPrint('[MessageCoordinator] Adding user message: "${content.substring(0, content.length.clamp(0, 50))}..."');
    }
  ...
  TherapyMessage addAIMessage(String content) {
    if (kDebugMode) {
      debugPrint('[MessageCoordinator] Adding AI message: "${content.substring(0, content.length.clamp(0, 50))}..."');
    }
  ...
  TherapyMessage addMessage(TherapyMessage message) {
    if (kDebugMode) {
      debugPrint('[MessageCoordinator] Adding pre-built message with sequence: ${message.sequence}');
    }
  ...
  void updateMessages(List<TherapyMessage> messages, int sequence) {
    if (kDebugMode) {
      debugPrint('[MessageCoordinator] Updating messages list with ${messages.length} messages, sequence: $sequence');
    }
  ...
  Map<String, dynamic> getConversationSummary() {
    return {
      ...
      'conversationDuration': hasMessages 
        ? _messages.last.timestamp.difference(_messages.first.timestamp).inMinutes
        : 0,
    };
  }
