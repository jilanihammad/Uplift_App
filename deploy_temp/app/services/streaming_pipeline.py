...
from typing import Dict, Any, Optional, AsyncGenerator, List, Callable, Tuple, Union, Deque
...
class LookaheadBuffer:
    """
    Small buffer for lookahead chunk processing with completion detection.
    Enhanced with performance monitoring capabilities.
    
    Features:
    - Configurable buffer size (default: 3 chunks)
    - Immediate streaming with delayed completion detection
    - Memory-efficient circular buffer
    - Automatic chunk metadata correction
    - Performance metrics tracking
    """
    
    def __init__(self, buffer_size: int = 3, enable_metrics: bool = True):
        """
        Initialize lookahead buffer with performance monitoring.
        
        Args:
            buffer_size: Number of chunks to buffer for lookahead (default: 3)
        ...
        self.avg_buffer_fill = 0.0
        self.max_buffer_fill = 0
        self.chunk_send_times = []
        
    def add_chunk(self, chunk_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Add chunk to buffer and return chunk ready for sending.
        Enhanced with performance tracking.
        
        Args:
            chunk_data: Raw chunk data from TTS stream
            
        Returns:
            Optional[Dict]: Chunk ready for sending, or None if buffer not full
        """
        # Add sequence number
        chunk_data["sequence"] = self.total_chunks_generated
        ...
        if self.enable_metrics:
            ...
            self.max_buffer_fill = max(self.max_buffer_fill, current_fill)
            
            # Update rolling average of buffer fill
            self.avg_buffer_fill = (
                0.9 * self.avg_buffer_fill + 
                0.1 * (current_fill / self.buffer_size)
            )
            
            # Track overflow (when buffer is already full)
        ...
        # Return oldest chunk if buffer is full
        if len(self.buffer) >= self.buffer_size:
            return self._prepare_chunk_for_sending()
        
        return None
    
    def mark_stream_end(self) -> List[Dict[str, Any]]:
        """
        Mark stream as ended and return all remaining chunks with correct metadata.
        
        Returns:
            List[Dict]: All remaining chunks with correct completion flags
        """
        ...
        while self.buffer:
            chunk = self._prepare_chunk_for_sending(force_send=True)
            if chunk:
                remaining_chunks.append(chunk)
        
        return remaining_chunks
    
    def _prepare_chunk_for_sending(self, force_send: bool = False) -> Optional[Dict[str, Any]]:
        """
        Prepare chunk for sending with correct metadata.
        
        Args:
            force_send: Force sending even if buffer not full
            
        Returns:
            Optional[Dict]: Chunk ready for sending
        """
        if not self.buffer:
            return None
        
        if not force_send and len(self.buffer) < self.buffer_size and not self.stream_ended:
            return None
        ...
        # Update metadata
        chunk["chunk_index"] = self.chunks_sent
        chunk["is_final_chunk"] = is_final_chunk
        chunk["is_sentence_end"] = is_final_chunk
        chunk["is_response_end"] = is_final_chunk
        
        # Add estimated total (will be accurate for final chunk)
        if is_final_chunk:
            chunk["total_chunks"] = self.chunks_sent + 1
        else:
            chunk["total_chunks_estimate"] = self.total_chunks_generated + len(self.buffer)
        
        self.chunks_sent += 1
        return chunk
    
    def get_performance_metrics(self) -> Dict[str, Any]:
        """Get buffer performance metrics for monitoring and optimization"""
        ...
        return {
            ...
            "avg_buffer_fill_ratio": self.avg_buffer_fill,
            "max_buffer_fill": self.max_buffer_fill,
            "efficiency_ratio": self.chunks_sent / max(1, self.total_chunks_generated),
            "overflow_ratio": self.buffer_overflow_count / max(1, self.total_chunks_generated)
        }


@dataclass 
class LookaheadConfig:
    """Enhanced configuration for lookahead streaming"""
    buffer_size: int = 3                    # Number of chunks to buffer
    max_latency_ms: int = 100              # Max acceptable latency increase
    enable_adaptive_buffering: bool = True  # Adapt buffer size based on network
    min_buffer_size: int = 2               # Minimum buffer size
    max_buffer_size: int = 5               # Maximum buffer size
    
    # ✅ NEW: Performance tuning options
    enable_buffer_preallocation: bool = True   # Pre-allocate buffer for performance
    enable_streaming_metrics: bool = True      # Track streaming performance metrics
    chunk_timeout_ms: int = 5000              # Timeout for stuck chunks
    
    def get_production_config(self) -> "LookaheadConfig":
        """Get production-optimized lookahead configuration"""
        return LookaheadConfig(
            buffer_size=4,                      # Slightly larger buffer for production
            max_latency_ms=150,                 # Allow slightly higher latency for stability
            enable_adaptive_buffering=True,
            min_buffer_size=3,                  # Higher minimum for production
            max_buffer_size=6,                  # Higher maximum for poor networks
            enable_buffer_preallocation=True,
            enable_streaming_metrics=True,
            chunk_timeout_ms=3000              # Shorter timeout for production
        )


def get_adaptive_buffer_size(network_quality: str, config: LookaheadConfig) -> int:
    """
    Get adaptive buffer size based on network quality with enhanced logging.
    
    Args:
        network_quality: Network quality assessment
        config: Lookahead configuration
        
    Returns:
        int: Optimal buffer size for the network conditions
    """
    if not config.enable_adaptive_buffering:
        return config.buffer_size
    
    # Adapt buffer size based on network quality
    size_mapping = {
        "poor": config.max_buffer_size,      # More buffering for poor networks
        "fair": config.buffer_size,          # Default buffering
        "good": config.min_buffer_size,      # Less buffering for good networks
        "excellent": config.min_buffer_size  # Minimal buffering for excellent networks
    }
    
    selected_size = size_mapping.get(network_quality, config.buffer_size)
    
    # ✅ ENHANCED: Log buffer size selection for debugging
    logger = logging.getLogger(__name__)
    logger.debug(
        f"Adaptive buffer sizing: {network_quality} network → {selected_size} chunks "
        f"(range: {config.min_buffer_size}-{config.max_buffer_size})"
    )
...
class EnhancedAsyncPipeline:
    ...
    def __init__(self, config: FlowControlConfig, llm_manager: LLMManager, lookahead_config: Optional[LookaheadConfig] = None):
        """
        ...
        Args:
            ...
            lookahead_config: Configuration for lookahead streaming (optional)
        """
        self.config = config
        self.llm_manager = llm_manager
        self.lookahead_config = lookahead_config or LookaheadConfig()
        self.logger = logging.getLogger(__name__)
    ...
    async def _process_tts_chunk(self, text_chunk: TextChunk, conversation_voices: Dict[str, str]) -> None:
        """
        IMPROVED: Process TTS chunk with lookahead buffering for low latency.
        
        Features:
        - Immediate chunk sending (maintains low latency)
        - Lookahead buffer for accurate completion detection
        - Memory-efficient streaming (no full collection)
        - Accurate metadata with minimal delay
        
        Args:
            text_chunk: Text chunk to convert to audio
            conversation_voices: Voice mapping for conversation consistency
        """
        sentence_id = text_chunk.metadata.get("sentence_id", "unknown")
        conversation_id = text_chunk.metadata.get("conversation_id", "default")
        
        # Get message-level metadata for tracking completion
        message_id = text_chunk.metadata.get("message_id", sentence_id.split("_")[0] if "_" in sentence_id else "unknown")
        is_response_end = text_chunk.metadata.get("is_response_end", False)  # 🎯 NEW: Track when response ends
        
        # Build conversation context for voice consistency
        conversation_context = {
            "conversation_id": conversation_id,
            "message_id": message_id,
            "sentence_id": sentence_id,
            "sequence": text_chunk.sequence_id,
            "timestamp": text_chunk.metadata.get("timestamp", datetime.now().isoformat()),
            "is_final_sentence": is_response_end  # 🎯 NEW: Mark final sentence
        }
        
        chunk_timestamp = time.time()
        
        # Network quality assessment for format optimization
        network_quality = "good"  # Default assumption
        if hasattr(self, '_last_client_metrics') and self._last_client_metrics:
            network_quality = self.assess_network_quality(self._last_client_metrics)
        
        # Select optimal audio format based on network conditions
        optimal_format = self.select_optimal_format(network_quality, {})
        format_params = self.get_format_parameters(optimal_format)
        
        # Voice consistency selection
        voice = self._get_consistent_voice(conversation_id, 
                                         text_chunk.metadata.get("voice_consistency_seed", ""), 
                                         conversation_voices)
        
        # Build TTS parameters
        tts_params = {
            "voice": voice,
            "response_format": optimal_format,
            "speed": 1.0,
            "pitch": 1.0,
            "conversation_id": conversation_id,
            **text_chunk.metadata.get("tts_params", {})
        }
        
        try:
            # ✅ ADAPTIVE BUFFERING: Adjust buffer size based on network quality
            buffer_size = get_adaptive_buffer_size(network_quality, self.lookahead_config)
            
            lookahead_buffer = LookaheadBuffer(
                buffer_size=buffer_size, 
                enable_metrics=self.lookahead_config.enable_streaming_metrics
            )
            first_chunk_sent = False
            
            self.logger.info(f"🚀 Starting lookahead TTS streaming with {buffer_size}-chunk buffer for '{text_chunk.text[:30]}...'")
            ...
            async for audio_chunk_b64 in self._stream_tts_audio(text_chunk.text, tts_params):
                ...
                # 🚀 Add to lookahead buffer and get chunk ready for sending
                ready_chunk = lookahead_buffer.add_chunk(chunk_data)
                
                if ready_chunk:
                    # 🚀 IMMEDIATE SENDING: Send chunk without waiting for full collection
                    success = await self._send_audio_chunk_immediate(ready_chunk, text_chunk)
                    ...
                    if success:
                        ...
                        if not first_chunk_sent:
                            ...
                            self.metrics.update_timing("time_to_first_audio_ms", ttfa_ms)
                            # ✅ ENHANCED: Include buffer info in logging
                            self.logger.info(
                                f"🚀 First audio sent: {ttfa_ms:.1f}ms "
                                f"(lookahead streaming, {buffer_size}-chunk buffer)"
                            )
                            first_chunk_sent = True
                    else:
                        self.logger.warning(f"Failed to send audio chunk {ready_chunk['chunk_index']}")
                        break
            
            # 🚀 COMPLETION: Send remaining buffered chunks with correct metadata
            remaining_chunks = lookahead_buffer.mark_stream_end()
            
            for chunk_data in remaining_chunks:
                if self.shutdown_event.is_set():
                    break
                    
                success = await self._send_audio_chunk_immediate(chunk_data, text_chunk)
                
                if success:
                    self.metrics.chunks_generated += 1
                else:
                    self.logger.warning(f"Failed to send final chunk {chunk_data['chunk_index']}")
                    break
            
            # Update final TTS latency timing
            total_latency = (time.time() - chunk_timestamp) * 1000
            self.metrics.update_timing("avg_tts_latency_ms", total_latency)
            
            # Log completion with buffer efficiency metrics
            total_chunks_sent = lookahead_buffer.chunks_sent
            buffer_efficiency = (total_chunks_sent / lookahead_buffer.total_chunks_generated) * 100 if lookahead_buffer.total_chunks_generated > 0 else 0
            
            self.logger.info(
                f"✅ TTS completed for '{text_chunk.text[:30]}...': "
                f"{total_chunks_sent} chunks sent, {total_latency:.1f}ms, "
                f"format: {optimal_format}, lookahead streaming "
                f"(buffer efficiency: {buffer_efficiency:.1f}%)"
            )
            
            # Log buffer performance metrics if enabled
            if self.lookahead_config.enable_streaming_metrics:
                buffer_metrics = lookahead_buffer.get_performance_metrics()
                self.logger.debug(f"Buffer performance: {buffer_metrics}")
            
            # 🎯 FIXED: Only send completion sentinel if this is the FINAL sentence of the response
            if is_response_end:
                self.logger.info(f"🏁 Sending completion signal for FINAL sentence of message {message_id}")
                
                # Send completion sentinel with accurate count
                completion_sentinel = CompletionSentinel(
                    request_id=message_id,  # Use message_id, not sentence_id
                    conversation_id=conversation_id,
                    total_chunks=total_chunks_sent,  # Accurate count for this sentence
                    metadata={
                        "sentence_id": sentence_id,
                        ...
                        "streaming_method": "lookahead_buffering",
                        "buffer_efficiency": buffer_efficiency,
                        "buffer_metrics": lookahead_buffer.get_performance_metrics() if self.lookahead_config.enable_streaming_metrics else {},
                        "is_final_completion": True  # Mark this as the final completion
                    }
                )
                ...
                if success:
                    self.logger.debug(f"✅ Final completion sentinel sent for {completion_sentinel.request_id}")
                else:
                    self.logger.warning(f"❌ Failed to send final completion sentinel for {completion_sentinel.request_id}")
            else:
                self.logger.debug(f"🔄 Sentence complete but not final: {sentence_id} (waiting for more sentences)")
        
        except Exception as e:
            # ✅ ENHANCED: Ensure buffered chunks are sent even on error
            self.logger.error(f"❌ TTS processing error, attempting to send buffered chunks: {str(e)}")
            
            try:
                # Try to send any buffered chunks before failing
                remaining_chunks = lookahead_buffer.mark_stream_end()
                for chunk_data in remaining_chunks:
                    try:
                        await self._send_audio_chunk_immediate(chunk_data, text_chunk)
                        self.metrics.chunks_generated += 1
                    except:
                        break  # Don't let cleanup errors mask the original error
            except:
    ...
    async def _send_audio_chunk_immediate(self, chunk_data: Dict[str, Any], text_chunk: TextChunk) -> bool:
        """
        Send audio chunk immediately with proper AudioChunk structure.
        
        Args:
            chunk_data: Processed chunk data from lookahead buffer
            text_chunk: Original text chunk for metadata
            
        Returns:
            bool: True if sent successfully
        """
        try:
            # ✅ FIX: Create clean metadata without bytes objects
            clean_metadata = {
                key: value for key, value in chunk_data.items() 
                if key != "audio_data"  # 🎯 Exclude bytes data from metadata
            }
            
            # Create AudioChunk with correct metadata
            audio_chunk = AudioChunk(
                chunk_id=f"{chunk_data['sentence_id']}_{chunk_data['chunk_index']}",
                sentence_id=chunk_data["sentence_id"],
                sequence=text_chunk.sequence_id,
                audio_data=chunk_data["audio_data"],  # bytes object (not in metadata)
                is_sentence_end=chunk_data["is_final_chunk"],  # Only final chunk marked as end
                boundary_type=text_chunk.boundary_type,
                metadata={
                    **clean_metadata,  # ✅ FIXED: No bytes objects in metadata
                    "is_realtime": True,
                    "streaming_method": "lookahead_buffering",
                    "estimated_bitrate_kbps": chunk_data["format_params"]["estimated_bitrate_kbps"],
                    "audio_size_bytes": len(chunk_data["audio_data"])  # Include size info instead
                }
            )
            
            # Send to client queue
            return await self._add_to_client_queue(audio_chunk)
            
        except Exception as e:
            self.logger.error(f"Error in immediate chunk sending: {e}")
            return False
...
async def create_pipeline(config: Optional[FlowControlConfig] = None, llm_manager: Optional[LLMManager] = None, lookahead_config: Optional[LookaheadConfig] = None) -> EnhancedAsyncPipeline:
    """
    ...
    Args:
        ...
        lookahead_config: Optional lookahead streaming configuration
    ...
    pipeline = EnhancedAsyncPipeline(config, llm_manager, lookahead_config)
    await pipeline.start()
    return pipeline


async def create_lookahead_pipeline(config: Optional[FlowControlConfig] = None, 
                                  lookahead_config: Optional[LookaheadConfig] = None,
                                  llm_manager: Optional[LLMManager] = None) -> EnhancedAsyncPipeline:
    """
    Create pipeline with lookahead streaming enabled for optimal performance.
    
    Args:
        config: Flow control configuration
        lookahead_config: Lookahead streaming configuration
        llm_manager: LLM manager instance
        
    Returns:
        Enhanced pipeline with lookahead streaming
    """
    if config is None:
        config = get_default_config()
    ...
    # Create pipeline with lookahead config
    pipeline = EnhancedAsyncPipeline(config, llm_manager, lookahead_config)
    await pipeline.start()
    return pipeline
...
def get_production_lookahead_config() -> LookaheadConfig:
    """Get production-optimized lookahead configuration for enhanced performance"""
    return LookaheadConfig(
        buffer_size=4,                      # Slightly larger buffer for production
        max_latency_ms=150,                 # Allow slightly higher latency for stability
        enable_adaptive_buffering=True,
