...
def test_tts_request(base_url, text, params, filename, test_dir):
            ...
            try:
                ...
                if audio_url:
                    ...
                    if audio_response.status_code == 200:
                        ...
                        file_path = test_dir / f"{filename}{extension}"
...
def main():
    """Run the TTS tests"""
    parser = argparse.ArgumentParser(description='Test audio compression settings')
    parser.add_argument('--url', type=str, default="https://ai-therapist-backend-385290373302.us-central1.run.app", 
                      help='Backend URL')
    ...
    print(f"Using backend URL: {args.url}")
    ...
    # Run each test
    for test in tests:
        result = test_tts_request(args.url, text, test["params"], test["filename"], test_dir)
        if result["success"]:
